...
assert.ok(upstreamRoot, 'usage: diagnose-upstream-sources.mjs <quint-checkout>')
assert.equal(
...
function contextFor(source, row) {
  return source.split(/\r?\n/)
    .slice(Math.max(0, row - 1), row + 2)
    .map(line => line.trim())
    .join(' ⏎ ')
}
...
for (const entry of manifest.validSources) {
  ...
  if (bad) failures.push({
    path: entry.path,
    type: bad.isMissing ? 'MISSING' : bad.type,
    row: bad.startPosition.row + 1,
    column: bad.startPosition.column + 1,
    context: contextFor(source, bad.startPosition.row),
  })
}

failures.sort((left, right) => left.path < right.path ? -1 : left.path > right.path ? 1 : 0)
for (const failure of failures) {
  console.log(`${failure.path}:${failure.row}:${failure.column}:${failure.type}:${failure.context}`)
}
